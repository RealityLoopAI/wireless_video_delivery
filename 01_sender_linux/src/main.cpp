...
void update_media_outage_guard(CameraRuntime &camera, Logger &logger, const CameraPerfStats &perf) {
    const bool rgb_preview_transport_healthy = camera.live.rgb_input_fps >= capture_outage_fps_floor()
                                               && perf.rgb_preview_sent_packets > 0 && perf.rgb_preview_send_failures == 0;
    const bool rgb_outage = camera.live.rgb_input_fps >= capture_outage_fps_floor() && perf.rgb_sent_packets == 0
                            && perf.rgb_send_failures > 0 && !rgb_preview_transport_healthy;
    const bool rgb_transport_healthy =
        camera.live.rgb_input_fps >= capture_outage_fps_floor() && (perf.rgb_sent_packets > 0 || rgb_preview_transport_healthy);
...
std::vector<uint8_t> q8_depth_indices_payload(const void *data, size_t size, uint32_t raw_step) {
    if(size == 0 || size % sizeof(uint16_t) != 0) {
        throw std::runtime_error("q8 depth compression requires uint16 payload");
    }
    const size_t sample_count = size / sizeof(uint16_t);
    if(sample_count > static_cast<size_t>(std::numeric_limits<uint32_t>::max())
       || sample_count > static_cast<size_t>(std::numeric_limits<int>::max())) {
        throw std::runtime_error("q8 depth payload too large");
    }
    ...
    for(size_t i = 0; i < sample_count; ++i) {
        const uint32_t value = read_depth_u16le(bytes, i);
        if(value == 0) {
            quantized[i] = 0;
            continue;
        }
...
std::vector<uint8_t> q8_compress_payload(const void *data, size_t size, uint32_t raw_step, uint32_t magic, bool use_lz4) {
    ...
    auto compressed = use_lz4 ? lz4_compress_payload(quantized.data(), quantized.size())
                              : zlib_compress_payload(quantized.data(), quantized.size());

    if(compressed.size() > static_cast<size_t>(std::numeric_limits<uint32_t>::max())) {
        throw std::runtime_error("q8 compressed depth payload too large");
    }
    ...
    append_le32(out, static_cast<uint32_t>(quantized.size()));
    append_le32(out, static_cast<uint32_t>(compressed.size()));
    append_bytes(out, compressed.data(), compressed.size());
    return out;
}

std::vector<uint8_t> q8lz4_compress_payload(const void *data, size_t size, uint32_t raw_step) {
    return q8_compress_payload(data, size, raw_step, 0x314c3851u, true);  // bytes: Q 8 L 1
}

std::vector<uint8_t> q8zlib_compress_payload(const void *data, size_t size, uint32_t raw_step) {
...
template <typename Sender>
bool send_media_packet(Sender &transport, const MediaPacketView &packet, StreamType) {
    return transport.send_media(packet);
}

bool send_media_packet(Transport &transport, const MediaPacketView &packet, StreamType stream_type) {
...
void media_sender_loop(LatestMediaQueue &media_queue, Sender &transport, Logger &logger, std::mutex &transport_mutex) {
    while(g_running) {
        ...
        {
            std::lock_guard<std::mutex> lock(transport_mutex);
            sent = send_media_packet(transport, packet, job->stream_type);
            if(!sent) {
    ...
    while(g_running) {
        ...
        if(color) {
            ...
            if(rgb_usable) {
                if(color_is_mjpg && preview_due) {
                    ...
                    record_rgb_decode_ms(camera, elapsed_ms(decode_started, std::chrono::steady_clock::now()));
                    set_latest_bgr(camera, preview_bgr, color->frame_id, color->system_timestamp_us);
                }
                else if(!color_is_mjpg) {
                    const auto decode_started = std::chrono::steady_clock::now();
                    ...
                    record_rgb_decode_ms(camera, elapsed_ms(decode_started, std::chrono::steady_clock::now()));
                    if(preview_due) {
                        set_latest_bgr(camera, bgr, color->frame_id, color->system_timestamp_us);
                    }
                }
        ...
        if(depth) {
            ...
            if(publish_depth) {
                ...
                meta.height = depth->height;
                meta.pixel_format = PixelFormat::depth_u16;
                meta.payload_size = depth_payload.size();
                meta.uncompressed_size = depth->data.size();
                auto job = make_owned_media_job(camera, StreamType::depth_raw, meta, std::move(depth_payload));
                publish_media_job(media_queue, depth_slot, camera, StreamType::depth_raw, std::move(job), logger);
