...
chmod +x "$WATCHDOG"
rm -f "$CHILD_PID_FILE"
export GEMINI_SENDER_MEDIA_OUTAGE_RESTART_SAMPLES="${GEMINI_SENDER_MEDIA_OUTAGE_RESTART_SAMPLES:-0}"
...
echo "发送端预览守护已启动，watchdog PID=$(cat "$PID_FILE")，DISPLAY=$DISPLAY_VALUE"
