...
5. 录制时先写 receiver 本地 staging；独立 uploader 以大块顺序流水镜像追随活动分片，校验后原子发布到 NAS，默认 `/home/fz/Desktop/nas`。NAS 异常时本地文件仍是权威副本。
...
13. `gwv3-recording-uploader.service` 与 receiver 分离运行；录制期间仅跟进当前热分片，不回读历史冷分片。NAS 临时不可用时保留本地 backlog，上传恢复不阻塞媒体线程。
