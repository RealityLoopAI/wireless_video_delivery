...
干净二进制先原子替换到两台正式路径。替换时 receiver 正在录制，因此没有立即重启正在发送的旧进程；录制结束且接收端收尾队列归零后，再重启两台服务。接收端最终确认两路均上报 `build_commit=bcc29464b2b5`、`build_dirty=false`，媒体恢复约 30 FPS，未引入录制中断。
