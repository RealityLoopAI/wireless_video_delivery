...
- 只有完整、顺序正确且无额外词的 `你好小环 / 您好小环` 才能触发。
- 默认关闭 `你好` 和 `小环` 的跨片段命中。
- 没有静音结尾或超过 3.2 秒的连续片段不送入唤醒匹配。
- 服务启动时把当前 USB 麦克风固定为 50% 捕获增益并关闭硬件 AGC。
- 低质量片段先过滤，不再全部送进 Vosk。
- 本地回复和 HTTP 文本播报统一进入 FIFO；播放前停止麦克风采集管线，Vosk 与
  RTP/Opus 同时暂停，队列清空 200ms 后再恢复。
...
实测硬件 AGC 会把当前设备的背景输入持续抬高到 VAD 人声区间，因此正式服务会关闭硬件 AGC。FFmpeg 的 `voice-agc` 同样不作为默认。默认前端能覆盖基础降噪，但不等同于真正 AEC。真正 AEC 需要拿到扬声器参考信号，建议后续二选一：
