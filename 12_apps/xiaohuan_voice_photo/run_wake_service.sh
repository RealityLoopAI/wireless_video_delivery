...
AUDIO_ARCHIVE_STREAM_ENABLED="${XIAOHUAN_AUDIO_ARCHIVE_STREAM_ENABLED:-0}"
AUDIO_ARCHIVE_HOST="${XIAOHUAN_AUDIO_ARCHIVE_HOST:-auto}"
AUDIO_ARCHIVE_PORT="${XIAOHUAN_AUDIO_ARCHIVE_PORT:-$AUDIO_STREAM_PORT}"
AUDIO_ARCHIVE_TIMING_PORT="${XIAOHUAN_AUDIO_ARCHIVE_TIMING_PORT:-50130}"
AUDIO_ARCHIVE_CONTROL_PORT="${XIAOHUAN_AUDIO_ARCHIVE_CONTROL_PORT:-50131}"
AUDIO_ARCHIVE_SENDER_ID="${XIAOHUAN_AUDIO_ARCHIVE_SENDER_ID:-$(hostname)}"
AUDIO_CAPTURE_REBUILD_STALE_SECONDS="${XIAOHUAN_AUDIO_CAPTURE_REBUILD_STALE_SECONDS:-2.0}"
...
PHOTO_DECODE_MIN_SECONDS="${XIAOHUAN_PHOTO_DECODE_MIN_SECONDS:-0.30}"
...
PHOTO_SENDER_ID="${XIAOHUAN_PHOTO_SENDER_ID:-*}"
PHOTO_CAMERA_ID="${XIAOHUAN_PHOTO_CAMERA_ID:-cam01}"
...
if [[ "$AUDIO_ARCHIVE_STREAM_ENABLED" == "1" ]]; then
  RECEIVER_RESOLVER="$ROOT_DIR/../recording_buttons/receiver_endpoint.py"
  if [[ -f "$RECEIVER_RESOLVER" ]]; then
    RESOLVED_AUDIO_ARCHIVE_HOST="$(python3 "$RECEIVER_RESOLVER" "$AUDIO_ARCHIVE_HOST" --host-only 2>/dev/null || true)"
    if [[ -n "$RESOLVED_AUDIO_ARCHIVE_HOST" ]]; then
      AUDIO_ARCHIVE_HOST="$RESOLVED_AUDIO_ARCHIVE_HOST"
    fi
  fi
fi
...

{
  ...
  if [[ "$AUDIO_STREAM_ENABLED" == "1" ]]; then
    ...
    if [[ "$AUDIO_ARCHIVE_STREAM_ENABLED" == "1" ]]; then
      AUDIO_STREAM_ARGS+=(
        ...
        --audio-archive-host "$AUDIO_ARCHIVE_HOST"
        --audio-archive-port "$AUDIO_ARCHIVE_PORT"
        --audio-archive-timing-port "$AUDIO_ARCHIVE_TIMING_PORT"
        --audio-archive-control-port "$AUDIO_ARCHIVE_CONTROL_PORT"
        --audio-archive-sender-id "$AUDIO_ARCHIVE_SENDER_ID"
      )
    else
      AUDIO_STREAM_ARGS+=(--no-audio-archive-stream)
    fi
  fi
  ...
  exec python3 -u vosk_wake.py listen \
    ...
    --audio-capture-rebuild-stale-seconds "$AUDIO_CAPTURE_REBUILD_STALE_SECONDS" \
    ...
    --photo-sender-id "$PHOTO_SENDER_ID" \
    --photo-camera-id "$PHOTO_CAMERA_ID" \
    ...
    --photo-decode-min-seconds "$PHOTO_DECODE_MIN_SECONDS" \
