...
同一相机同一秒内重名时追加 `_001`、`_002`。目录时间来自实际选中 RGB 帧的 `frame_system_timestamp_us`，不是语音识别完成时间或 NAS 上传时间。
