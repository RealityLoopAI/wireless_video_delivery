...
- 页面可见时，主画面与 RGB 小窗口均以 30 FPS 为目标。安全上下文优先使用 WebCodecs；普通 HTTP 页面的小窗口使用浏览器原生 fMP4 视频。fMP4 优先重新封装 Sender 的低码率 H.264 预览流；预览流启动失败时复用现成主 H.264，二者都不解码、不重编码。浏览器断开时异步清理对应 FFmpeg，视频不可用时才回退到单请求 JPEG。网络变慢时预览会丢过期帧而不堆积；Depth 保持较低刷新率以控制接收端负载。
