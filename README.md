...
3. Depth 以 `uint16` 深度帧为母版，当前支持原始帧或 zlib 无损压缩后发送。
4. 接收端接收媒体数据和状态心跳，提供录制控制和网页预览。
5. 录制数据写入接收端挂载的 NAS 目录。
6. Web Monitor 用于看在线状态、实时预览、开始/停止录制和设置显示名称。
...
07_samples/          样例材料
08_reports/          运行时日志目录，历史报告已迁入 04_docs/archive
