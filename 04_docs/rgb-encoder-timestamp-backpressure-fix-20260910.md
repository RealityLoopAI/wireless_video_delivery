...
本机 12/12 编码回归已通过。设备原生 MPP 验证、实际部署及 NAS 录制结果在后续验收补充，
不能将软件 x264 测试等同于现场 MPP 已验收。
...
上线前确认 Receiver 空闲，保留目标机原二进制及 MPP 插件覆盖配置；逐台本机构建，
确认上报 commit、RGBD 帧率和时钟模型。异常时恢复本轮二进制备份，重启对应服务。
