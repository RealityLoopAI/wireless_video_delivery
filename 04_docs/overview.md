...
NAS 是录制文件的最终存放位置。当前生产配置先在 Receiver 本地 staging 写分片，再由 uploader 增量镜像到 NAS 隐藏队列；关闭 fragmented MP4、Depth、CSV 和元数据后校验并原子发布到正式目录。该链路不做 RGB 整文件重封装，并允许 NAS 短时离线后自动补传。直接写 NAS 仅保留为受控环境兼容模式。
