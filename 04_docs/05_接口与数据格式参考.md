...
`capture_not_before_unix_us` 为 sender 本机时间轴上的最早采集时刻。语音服务一次写入三张请求，并用该字段表达 0.2 秒间隔，避免目录扫描和回执延迟改变目标采集时间。
...
路径时间来自选中帧的 `system_timestamp_us`。同秒重名追加 `_001`。未配置软件旋转时保存相机原始 MJPEG 有效字节；配置 180 度软件旋转时，JPG 已按相同方向校正。
