...
`/api/status` 顶层包含 `build_commit`、`build_dirty`、`build_source_hash`、`record_queue_total_bytes`；每路相机包含 `record_accepting`、`record_finalizing`、队列峰值和写入错误计数，可用于部署核对和录制健康检查。`build_source_hash` 由 sender、receiver、common C++ 源码和 CMake 内容计算，不受现场相机配置差异影响。
