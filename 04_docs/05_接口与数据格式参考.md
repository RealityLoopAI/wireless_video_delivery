...
| 4 | `rgb_snapshot` | 语音拍照按需发送的单帧相机原始 MJPEG；使用独立可靠发送队列 |
...
语音服务向发送端本地请求目录写入 `rgb_snapshot_request`。发送端从目标相机取得下一帧完整 MJPEG，保留原始字节并通过 `rgb_snapshot` 包发送。接收端把图片和 CRC 清单写入本地 staging 并完成 `fsync` 后，通过 sender 的状态 UDP 回传：
...
```json
{
  ...
  "camera_id": "cam01",
  "request_id": "xiaohuan_photo_20260728_120000_000001",
  "ok": true,
  "status": "captured",
  "image_path": "/home/fz/Desktop/nas/voice_photos/orangepi5pro-d12a4719_cam01/2026-07-28/12-00-00/20260728_120000.jpg"
}
```

`captured` 只表示接收端本地副本已可靠持久化，可以播放“好的，已拍照”；不表示 NAS 上传已经结束。`gwv3-photo-uploader.service` 随后校验大小和 CRC，通过 NAS 端临时文件加原子重命名发布 JPG。NAS 不可用时保留 staging 并重试。
...
路径时间来自选中帧的 `system_timestamp_us`。同秒重名追加 `_001`。由于保存的是相机原始 MJPEG，发送端为视频预览配置的软件旋转不会写进 JPG。
...
录制期间不会公开最终名称的 `frames.csv`。接收端先在本地 staging 写 `frames.csv.inprogress`，收尾时合并 RGB 实际落盘索引，再原子发布 `frames.csv`、`meta.closed=true` 和 `recording_staged.json`。独立 uploader 先把原始 capture 包可靠复制到 NAS 隐藏队列并释放本地副本，再从 NAS 队列完成普通 MP4 转换、校验和最终目录发布。隐藏队列不会出现正式 `recording_ready.json`；该文件只在最终目录清理完成后原子生成。下游开始对齐前必须满足：
...
`recording_uploader` 使用 `gwv3_recording_uploader_status_v2`。除 `active_phase`、`active_bytes_done`、`active_bytes_total`、`active_progress_percent` 和 `active_elapsed_ms` 外，还提供 `local_pending_segments/bytes`、`nas_finalize_pending_segments/bytes`、`publish_recovery_journals`、`captured_segments` 和 `last_capture_success_us`。`local_pending_segments=0` 表示本地数据已被 NAS capture 安全接管；只有总 `pending_segments=0` 才表示普通 MP4 和最终 ready 已全部交付。
...
自动到达 `segment_seconds` 时，接收端先原子切换到新切片，再由后台 worker 快速关闭旧本地切片。显式停止时，`segment_finalizing/record_finalizing` 只表示 writer 分离的短临界区；分离后的后台工作由 `segment_finalize_pending` 和顶层 finalizer 计数表示，它不再阻塞新录制。NAS capture、普通 MP4 重封装和最终发布由独立 uploader 串行处理，且在 receiver 队列积压或切片边界附近主动暂停。`recording=true` 可以与 receiver finalizer 或 uploader backlog 同时出现。重启、断电或正式交付前必须等待 `record_finalize_outstanding_segments=0` 且 uploader backlog 为 0。
