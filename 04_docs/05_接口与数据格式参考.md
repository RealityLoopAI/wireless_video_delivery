...
`recording_uploader` 还包含 `active_phase`、`active_bytes_done`、`active_bytes_total`、`active_progress_percent` 和 `active_elapsed_ms`。它们分别表示当前处于普通 MP4 重封装、复制或校验阶段，以及该阶段的字节进度和持续时间；长文件处理时应先看这些字段是否持续变化，再判断进程是否卡死。
