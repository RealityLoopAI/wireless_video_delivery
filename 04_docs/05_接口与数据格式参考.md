...
| `cameras[].adaptive_exposure.enabled` | 启用源 MJPEG 软件测光与手动曝光闭环；默认关闭，目前仅用于 Gemini 305 |
| `cameras[].adaptive_exposure.interval_ms` | 测光周期，默认 500 ms，不进入逐帧编码路径 |
| `cameras[].adaptive_exposure.exposure_min/max` | 软件控制允许的曝光范围；为保证 Gemini 305 30fps，配置最大值不得超过 300 |
| `cameras[].adaptive_exposure.gain_min/max` | 软件控制允许的 RGB gain 范围 |
| `cameras[].adaptive_exposure.target_p95_luma` | 中央 ROI 的源 MJPEG 灰度 P95 目标值 |
...
`recording_uploader` 使用 `gwv3_recording_uploader_status_v2`。除 `active_phase`、`active_bytes_done`、`active_bytes_total`、`active_progress_percent` 和 `active_elapsed_ms` 外，还提供 `rgb_output_mode`、`fragmented_passthroughs`、`local_pending_segments/bytes`、`nas_finalize_pending_segments/bytes`、`publish_recovery_journals`、`captured_segments` 和 `last_capture_success_us`。`local_pending_segments=0` 表示本地数据已被 NAS capture 安全接管；只有总 `pending_segments=0` 才表示最终媒体和 ready 已全部交付。
