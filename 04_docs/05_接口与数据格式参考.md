...
是默认的低码率网页预览流；下游同时只拉一路且需要主码流画质时，使用 `quality=main`。主码流直接复用录制母流，不会
启动发送端低清预览编码，但会增加 receiver 到下游的局域网带宽。
...
| `recording_staging.full_copy_bandwidth_limit_mbps` | 无增量镜像可复用时的旧整文件独立限速；不影响活动镜像和高复用率新分片 |
...
| `recording_staging.full_copy_workers` | 无可复用增量镜像时的完整本地文件读取并发数；机械盘建议为 `1`，避免多路顺序文件变成随机读 |
| `recording_staging.parallel_capture_min_mirror_reuse_percent` | 不受完整拷贝并发限制所需的最小镜像复用率；默认 `90` |
...
| `cameras[].adaptive_exposure.interval_ms` | 测光周期，默认 500 ms，不进入逐帧编码路径 |
...
启用 `adaptive_exposure` 时，`color_controls.auto_exposure` 必须显式为 `false`，并提供位于自适应范围内的初始 `exposure` 和 `gain`。发送端 heartbeat 会额外上报 `adaptive_exposure_luma_p50/p95/p99`、`adaptive_exposure_highlight_fraction`、当前请求曝光/增益、样本数、调整数和失败数。测光或 SDK 属性写入失败时只保留当前手动参数，不会中断采集、编码或发送。
...
`meta.json`、`recording_staged.json` 和 `recording_ready.json` 还会保存分片有效窗口摘要：`recording_window_first_valid_global_us`、`recording_window_last_valid_global_us`、`recording_window_valid_rows`、`recording_window_valid_rgb_frames` 和 `recording_window_valid_depth_frames`。`meta.json` 额外保存 RGB/Depth 各自的首尾 `global_timestamp_us`。下游可先用摘要快速检查各路有效窗口是否重叠，再逐行消费 `recording_window_valid=1` 的 `frames.csv`；摘要不代替逐帧索引。
...
每路相机的切片边界状态包含 `segment_rotation_keyframe_requested_us`、`segment_rotation_keyframe_requests`、`segment_prestart_depth_drops` 和 `segment_prestart_rgb_drops`。前两项用于确认超时轮转是否正在等待可解码 RGB 关键帧，后两项记录 idle 重连或进程恢复后、首个 SPS/PPS/IDR 之前被拒绝的无配对 Depth 和不可独立解码 RGB。
...
自动到达 `segment_seconds` 时，接收端先原子切换到新切片，再由后台 worker 快速关闭旧本地切片。显式停止时，`segment_finalizing/record_finalizing` 只表示 writer 分离的短临界区；分离后的后台工作由 `segment_finalize_pending` 和顶层 finalizer 计数表示，它不再阻塞新录制。NAS capture、容器校验和最终发布由独立 uploader 处理，且在 receiver 队列积压或切片边界附近主动暂停。生产 fMP4 模式不执行 RGB 整文件重封装。`recording=true` 可以与 receiver finalizer 或 uploader backlog 同时出现。重启、断电或正式交付前必须等待 `record_finalize_outstanding_segments=0` 且 uploader backlog 为 0。
