...
`/api/status` 顶层包含 `build_commit`、`build_dirty`、组件级 `build_source_hash`、`record_queue_total_bytes`、`recording_staging_enabled`、`recording_write_root`、`recording_uploader`，以及 `record_finalize_outstanding_segments`、`record_finalize_queued_segments`、`record_finalize_active_segments`、`record_finalize_completed_segments`、`record_finalize_failed_segments`、`media_ingress_superseded_sessions`、`media_ingress_stale_packets`。每路相机包含 sender 组件哈希、输入/发送 FPS、发送失败计数、当前 media session、暖机状态、录制队列和 finalizer 指标。所有 sender 的 `build_source_hash` 应一致；receiver 使用独立哈希，不因另一组件源码变化产生假差异。

自动到达 `segment_seconds` 时，接收端先原子切换到新切片，再由后台 worker 快速关闭旧本地切片。普通 MP4 重封装和 NAS 发布由独立 uploader 串行处理。`recording=true` 可以与 receiver finalizer 或 uploader backlog 同时出现。显式停止、重启或升级 receiver 前必须等待 `record_finalize_outstanding_segments=0`；正式交付前还必须等待 uploader backlog 为 0。
