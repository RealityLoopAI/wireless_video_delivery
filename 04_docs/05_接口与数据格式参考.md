...
| `recording_staging.incremental_mirror_enabled` | 是否在活动分片上做增量镜像；当前单机磁盘方案默认关闭，避免多路小块随机读 |
| `recording_staging.full_copy_workers` | 已关闭分片的 NAS 搬运并发；当前空闲时为 2，录制时由 `active_recording_full_copy_workers=1` 限制 |
