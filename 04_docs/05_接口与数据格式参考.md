...
| `meta.json` | 录制元信息 |
| `recording_ready.json` | 该段媒体和最终帧索引均已完成发布的标志 |
...
从当前版本开始，录制期间不会公开最终名称的 `frames.csv`。接收端先写 `frames.csv.inprogress`，收尾时合并 RGB 实际落盘索引，再原子发布 `frames.csv`，随后将 `meta.json` 写为 `closed=true` 并生成 `recording_ready.json`。下游开始对齐前必须满足：
...
`/api/status` 顶层包含 `build_commit`、`build_dirty`、`build_source_hash`、`record_queue_total_bytes`，以及 `record_finalize_outstanding_segments`、`record_finalize_queued_segments`、`record_finalize_active_segments`、`record_finalize_completed_segments`、`record_finalize_failed_segments`、`media_ingress_superseded_sessions`、`media_ingress_stale_packets`。每路相机包含 `record_accepting`、`record_finalizing`、`segment_finalize_pending`、`segment_finalize_active`、`segment_finalize_completed`、`segment_finalize_failures`、`segment_finalize_last_duration_ms`、`record_queue_oldest_age_ms`、`record_prequeue_peak_delay_ms`、`record_queue_peak_wait_ms`、队列峰值和写入错误计数，可用于部署核对和录制健康检查。`build_source_hash` 由 sender、receiver、common C++ 源码和 CMake 内容计算，不受现场相机配置差异影响。

自动到达 `segment_seconds` 时，接收端先原子切换到新切片，再由全局单线程后台 worker 串行完成旧切片的 MP4 重封装、校验和原子发布。`recording=true` 可以与 `record_finalize_outstanding_segments>0` 同时出现，这是连续录制的正常状态。显式停止、重启或升级前必须等待 `record_finalize_outstanding_segments=0`。
