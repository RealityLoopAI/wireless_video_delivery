...
7. 到 NAS 目录检查 `rgb.mp4`、`depth.mkv`、`frames.csv`、`meta.json`、`recording_ready.json` 和 `calibration.json`。
8. 多机对齐前运行 `./05_tools/sync_input_guard.py --inputs <各路录制目录> --verify-video-frames`。
