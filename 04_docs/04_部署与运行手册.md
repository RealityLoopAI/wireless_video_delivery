...
4. 观察录制状态和保存目录。
5. 停止录制。
6. 等待页面中 `record_finalizing=false`、`segment_finalizing=false`、录制队列归零；此时可以开始下一次录制。
7. 等待 `recording_uploader.pending_segments=0`，再到 NAS 目录检查 `rgb.mp4`、`depth.mkv`、`frames.csv`、`meta.json`、`recording_ready.json` 和 `calibration.json`。
