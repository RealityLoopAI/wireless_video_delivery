...
recording_staging.retain_local_capture_for_finalize: capture 安全落 NAS 后暂留本地 fMP4 供重封装
...
recording_staging.finalize_workers: 本地源 RGB 并行重封装上限，生产默认 2
...
当前 `lubancat-52d2ef0c` 的处理方式是保留系统插件不动，在下面目录放置已验证为
GStreamer 1.20 ABI 的 Rockchip MPP 插件：
...
4. 观察录制状态和保存目录。
5. 停止录制。
6. 等待页面中 `record_finalizing=false`、`segment_finalizing=false`、录制队列归零；此时可以开始下一次录制，不必等待普通 MP4 后处理。
7. `local_pending_segments=0` 表示本地已关闭分片都已安全写入 NAS capture queue；`local_finalize_cache_segments` 是仍保留用于快速重封装的本地可删除缓存，不是唯一副本。
8. 正常重封装应看到 `last_remux_source=local`；本地缓存缺失或被磁盘水位清理后会显示 `nas`，表示已安全回退为 NAS 源。
