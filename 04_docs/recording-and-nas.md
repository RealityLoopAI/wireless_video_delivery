...
4. 实际录制目录的容量故障触发整次 session `faulted`，不在同一 session 中悄悄续录；其他真实写入失败保留原有错误上报和该路暂停处理。NAS 健康快照异常不能单独触发活动录制停录。

NAS 健康、开始新录制的准入、活动录制实际存储能力分别判断。NAS 快照不可写、缺失、过期或损坏时，已经开始的录制继续使用原 session 和切片时间轴；没有 NAS 异常自动停录宽限期。上传暂停并保留待交付数据，恢复后重试。新录制仍按原 NAS 准入规则处理，健康状态继续如实报告。行为矩阵、回归验证与适用边界见 [NAS 状态与活动录制解耦](nas-recording-decoupling-20260916.md)。
