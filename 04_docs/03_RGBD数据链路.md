...
sequenceDiagram
  ...
  S->>S: RGB 编码为 H.264
  S->>S: Depth 保持 uint16 或 zlib 无损压缩
  S->>R: TCP 媒体包
  S->>R: UDP 状态心跳
  R->>W: 状态和预览
  W->>R: 开始录制
...
| `system_timestamp_us` | 发送端主机系统时间戳，Unix epoch microseconds | 多发送端和下游对齐的优先字段 |
...
1. 先读取每路 `frames.csv`。
2. 筛选已录制 RGB 行。
3. 使用 `frame_system_timestamp_us` 作为优先对齐字段。
...
3. 优先按 `frame_system_timestamp_us` 做最近邻。
...
2. Web 预览不是正式数据依据。
3. 严格同步需要时间同步服务、稳定采集链路和离线验证共同保证。
4. 如果发送端系统时间未同步，`system_timestamp_us` 的跨设备意义会下降。
