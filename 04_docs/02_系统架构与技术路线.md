...
正式输出是录制文件，而不是实时 SDK 输出。NAS 目录的好处是：
...
NAS 可以通过 NFS、CIFS/SMB 或等价网络文件系统挂载，文档不再写死某一种协议。
...
| `02_receiver_linux` | 媒体接收、状态聚合、CLOCK_SYNC 模型、录制控制、预览派生、本地管理 API、NAS 落盘 | 修改发送端采集参数、分配正式 ID |
| `03_common_core` | 协议常量、媒体包头、公共结构 | 业务流程 |
| `05_tools` | 启停、状态、预检、导出、时间同步、维护脚本 | 核心媒体处理 |
