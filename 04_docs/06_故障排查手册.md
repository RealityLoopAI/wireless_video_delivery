...
2. `write_debug_h264=false` 时不再打开或双写 RGB debug H.264；只有显式开启才生成旁路。
3. 独立 uploader 在本地无损重封装普通 MP4，完成 `moov`、时长和 Depth 可读性检查。
4. NAS 复制进入隐藏临时目录，文件清单一致后原子改名，`recording_ready.json` 最后发布。
5. NAS 不可用或上传器重启时，本地完成分片保留并自动重试，不反压 receiver media socket。
6. 自动切片先切换到新 writer，旧本地 writer 由后台 worker 快速关闭；普通 MP4 收尾不占用 receiver cgroup。
7. sender 离线超过 `idle_finalize_ms` 时旧分片主动关闭，重连后进入新分片。
8. 停止 receiver 前应等待 receiver 队列和 finalizer 归零；正式交付或关机前还应等待 uploader backlog 为 0。
9. 文件带前缀时，ready 标记也带相同前缀；以 `meta.recording_ready_file` 为准。
