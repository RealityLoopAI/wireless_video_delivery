...
4. 接收端实时收尾路径不再执行 GB 级 retime/faststart 重封装，避免长切片被停止脚本或 systemd 打断后留下半成品。
5. 停止 receiver 前应先走 `/api/record/stop-all` 并等待 `/api/status` 中 `segment_active=false`、`segment_finalizing=false`、录制队列为空；当前 `stop_receiver.sh` 和自启动安装脚本已内置这个等待。
