...
6. 自动切片先切换到新 writer，再把旧段交给全局单线程后台收尾；多路相机不会同时在 NAS 上做完整 MP4 重封装。
7. 后台收尾队列由 `record_finalize_max_pending_segments` 有界控制。队列已满时只延后下一次自动轮转，当前切片继续录制，不把媒体 payload 无限堆在内存里。
8. 停止 receiver 前应先走 `/api/record/stop-all` 并等待 `/api/status` 中 `segment_active=false`、`segment_finalizing=false`、`record_finalizing=false`、录制队列为空且 `record_finalize_outstanding_segments=0`；当前 `stop_receiver.sh` 和自启动安装脚本已内置最长 900 秒等待。
9. 长切片停止后后台收尾状态会持续到无损重封装完成；此时不要重启或断电。systemd 单元的 `TimeoutStopSec` 也必须足以覆盖这段时间。
...
6. 若 `record_finalize_active_segments` 长期大于 1，说明运行的不是当前串行收尾版本；若 `record_finalize_outstanding_segments` 持续达到配置上限，检查 NAS 吞吐、CIFS 延迟和 ffmpeg 日志。
