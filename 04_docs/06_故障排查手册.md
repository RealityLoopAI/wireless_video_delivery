...
4. 独立 uploader 先把 closed fragmented MP4、Depth、CSV 和元数据复制到 NAS 隐藏 capture queue；文件逐个 `fsync`、清单一致且 capture 标记落盘后才删除本地副本。
5. 普通 MP4 无损重封装在 NAS capture 包上后台执行；内部 finalized 标记不是下游 ready。
