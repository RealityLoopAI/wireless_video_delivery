...
5. `12_build/web_auth_token` 是否存在且非空，Web unit 是否带 `GWV3_WEB_AUTH_TOKEN`。

如果 API 返回 `401`，重新打开首页并输入令牌，不要把 C++ admin 端口暴露到局域网规避认证。
...
1. RGB 使用 fragmented MP4 降低收尾失败风险。
2. 录制期间可保留 `rgb_debug.h264` 作为修复旁路。
3. 成功校验后 debug 文件默认不长期保留。
4. 接收端实时收尾路径不执行 GB 级 retime/faststart 重封装，避免长切片被停止脚本或 systemd 打断后留下半成品。
...
4. 检查 NAS 写入和系统是否杀进程。
5. 如果看到 `*.retime_replace_tmp.mp4`，说明是旧版本重封装阶段被打断留下的历史残留；新版本实时路径不会再生成该文件。
...
2. USB autosuspend。
3. SDK 版本或架构不匹配。
4. Gemini305 等设备需要 V4L2 适配方向，但当前主线未必完整合入。
5. `serial_number` 或 `uid` 配置不匹配。
