...
5. 普通 MP4 后台重封装优先从本地缓存读取、直接写入 NAS，避免 NAS 经 CIFS 读回 receiver 后再写回；本地缓存不存在时自动使用 NAS capture。内部 finalized 标记不是下游 ready。
...
6. 如果 NAS 的 `.gwv3_capture_queue` 中出现 capture/finalized 标记，说明数据已安全到 NAS、但普通 MP4 尚未最终发布；不得把隐藏目录直接交给下游。
...
16. uploader 正常应显示 `pipeline_mode=nas_first_local_cache_finalize` 和 `last_remux_source=local`。若持续显示 `nas`，检查本地是否达到缓存高水位、`recording_capture_cached.json` 是否存在，以及 staging 是否被外部清理。
