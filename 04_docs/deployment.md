...
本文面向安装和开发维护人员。非技术现场人员不要从本页抄命令，直接使用
[现场运维操作手册](operator-manual.md)。关于 DHCP、自动发现、NAS 和离线边界的方案答复见
[现场交付方案与问题答复](field-delivery-solution.md)。

新设备不再按下面各节逐项手工安装。正式初始化统一使用
[one-click-deployment.md](one-click-deployment.md)；本页保留组件级命令用于维护和排障。
