...
初次排查期间没有更换生产二进制、重启生产服务、切换 Wi-Fi、停止当前录制；
当时 sender 为 `b031e8ee2aed`，receiver 为 `ad80eb33f0b8`。
用户随后授权上线，确认停止及收尾完成后，已将 `9d2ef259766c` 部署到在线六台 sender 和 receiver。
各机原生 SDK 编译、二进制校验、短录制及残留积压风险见
[上线验收记录](clock-history-deployment.md)。不要把下文初次排查的访问限制当作当前状态。
...
初次排查没有完成 SDK v2.8.6 原机编译部署；后续上线已在两台 LubanCat 分别编译并通过 8/8 CTest。
无线限速压力测试、连续长录及内容级校准仍未完成。实机短测另见上线记录。
