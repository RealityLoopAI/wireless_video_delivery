...
NAS 是录制文件的最终存放位置。当前接收端先写本地 staging，分片关闭后由独立上传器校验、复制并原子发布到 NAS。生产配置直接交付正常关闭的 fragmented MP4，避免再次完整读取和重写 RGB 文件；需要兼容旧播放器时仍可配置为无损重封装普通 MP4。NAS 可以通过 NFS、CIFS/SMB 或现场等价网络文件系统挂载。
...
5. 你在网页上点开始录制。
6. 接收端为每路相机在本地 staging 创建录制目录。
7. RGB 写成 `rgb.mp4`。
8. Depth 写成 `depth.mkv`。
9. 帧索引、时间戳、`global_timestamp_us` 和诊断字段写进 `frames.csv`。
10. 相机参数写进 `calibration.json`。
11. 录制元信息写进 `meta.json`。
12. 你点停止录制后，接收端快速关闭本地文件并继续保持实时预览。
13. 独立上传器把 fragmented RGB MP4 转为普通可拖动 MP4，校验后发布到 NAS。
14. NAS 中最后出现 `recording_ready.json`，该段才可交给下游。
...
想理解为什么这么设计，读 [02_系统架构与技术路线.md](02_系统架构与技术路线.md)。

想理解数据怎么走，读 [03_RGBD数据链路.md](03_RGBD数据链路.md)。

想直接运行，读 [04_部署与运行手册.md](04_部署与运行手册.md)。
