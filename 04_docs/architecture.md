...
发送端和接收端的 `main.cpp` 只保留进程入口。应用生命周期在 `application.cpp`，原有私有实现按职责放在 `src/detail/*.inl`；这些文件仍在同一个 translation unit 内按固定顺序编译，因此重构不改变匿名命名空间、调用顺序和 ABI。后续只有在有独立接口与单元测试后，才把稳定边界迁移为独立 `.cpp` 模块。

源码版本哈希包含 `.inl`，部署状态不会因为实现移出 `main.cpp` 而漏报改动。
...
这些路线的详细优先级见 [roadmap.md](roadmap.md)。
